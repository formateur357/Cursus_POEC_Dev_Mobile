...
function shuffle() {
  array = document.querySelectorAll("div#jeu button");
  ...
  while (currentIndex != 0) {
    ...
    [array[currentIndex].innerHTML, array[randomIndex].innerHTML] = [array[randomIndex].innerHTML, array[currentIndex].innerHTML];

    [array[currentIndex].className, array[randomIndex].className] = [array[randomIndex].className, array[currentIndex].className];
  }
...
function winGame() {
  array = document.querySelectorAll("div#jeu .case");
  console.log(array);
  for (let i = 0; i < array.length - 1; i++) {
    if (parseInt(array[i].innerHTML) >= parseInt(array[i + 1].innerHTML)){
      console.log(i + " : " + array[i].innerHTML + " : " + array[i + 1].innerHTML);
      return ;
    }
    
  }
  var lambda = document.getElementById("victory")
  lambda.textContent = "Tu as gagné !!";
  lambda.style.textAlign = "center";
 lambda.style.marginTop = "50px";
...
img.src = "https://www.francetvinfo.fr/pictures/ckkdbadlH5EXJdG6y7UShk1FQFE/0x0:1378x775/1320x743/filters:format(webp)/2019/03/08/phpsVNfhD.jpg";
  for (let i = 0; i < array.length - 1; i++) {
